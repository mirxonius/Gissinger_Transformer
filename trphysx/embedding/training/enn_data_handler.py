...
class GissingerDataHandler(EmbeddingDataHandler):
    """
    Essentially this is the same class as the LorenzoDataHandler,
    but we are making a distinct class for clarity.
    """

    class GissingerDataset(Dataset):
        def __init__(self,examples:List):
                self.examples = examples
        ...
        def __getitem__(self,index)->Dict[str,torch.Tensor]:
            return {"states":self.examples[index]}


    @dataclass
    class GissingerDataCollator:

        def __call__(self,examples:List[Dict[str,torch.Tensor]])->Dict[str,torch.Tensor]:
            x_data_tensor = torch.stack([example["states"]
    ...
    def createTrainingLoader(self, 
        ...
        # Create dataset, collator, and dataloader
        dataset = self.GissingerDataset(data)
        data_collator = self.GissingerDataCollator()
        training_loader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=data_collator, drop_last=True)
    ...
    def createTestingLoader(self, 
        ...
        """Creating testing/validation data loader for Gissinger system.
        For a data case with time-steps [0,T], this method extract a smaller
        time-series to be used for testing [0, S], s.t. S < T.
...
LOADER_MAPPING = OrderedDict(
    [
        ...
        ("gissinger", GissingerDataHandler),
    ]
)
